...
const ShopContent = () => {
    ...
    // Fetch products from Supabase with search support
    const { products, isLoading, error } = useProducts({
        search: searchQuery
    });

    // Get filters from URL - now supporting multi-select
    ...
    // Create stone-to-benefit mapping
    ...
    // Get unique stone types and product types from products
    const stoneTypes = useMemo(() => {
        if (!products.length) return [];
        const types = new Set(products.map(p => {
            // Normalize: if Other, map to Opal
    ...
    // Compute stone availability per product type (for cross-out feature)
    ...
    // Compute type availability per stone (for bidirectional cross-out)
    ...
    // Derive benefits from selected stones
    ...
    const displayedProducts = useMemo(() => {
        ...
        if (activeStoneFilters.length > 0) {
            // More robust filter - also check if stone is in tags for fuzzy match
            filtered = filtered.filter(p => {
                const stoneMatch = activeStoneFilters.includes(p.stone);
                // Also match if filter is "Opal" and product has "opal" in tags but wrong stone field
    ...
    // Toggle filter for multi-select
    ...
    // Check if a stone is available in currently selected categories
    ...
    // Check if a category is available for currently selected stones
            ...
            <div className="container mx-auto px-6 py-4 md:py-6">
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-3 md:gap-4 mb-8 pb-4 md:pb-8 border-b border-gray-200 sticky top-20 z-30 bg-white/80 backdrop-blur-xl px-4 py-3 md:px-0 md:bg-transparent md:static rounded-xl md:rounded-none">
                    <div className="flex items-center justify-between w-full md:w-auto gap-2 md:gap-4">
                        ...
                        {/* Sort Dropdown - appears on same row on mobile */}
                ...
                {/* Filter Panel - Premium Design with Multi-Select */}
                <div className={`
                ...
                `}>
                    ...
                    `}>
                        ...
                        <div className="flex-1 overflow-y-auto p-6 md:p-0 md:overflow-visible">
                            ...
                            {/* Stone Type Filter - Multi-Select */}
                            <div>
                        ...
                        {/* Selected Stone Benefits Display */}
                ...
                {hasActiveFilters && !isFilterOpen && (
                    <div className="flex flex-wrap items-center gap-3 mb-8 animate-in fade-in slide-in-from-top-2 duration-300">
                        ...
                        {/* Benefits summary when filter panel is closed */}
                        {selectedBenefits.length > 0 && (
                ...
                {/* Load More Indicator (for future) */}
                {displayedProducts.length > 0 && (
...
export default function ShopPage() { return <React.Suspense fallback={<div>Loading shop...</div>}><ShopContent /></React.Suspense>; }
